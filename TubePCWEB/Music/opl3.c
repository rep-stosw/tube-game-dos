...
static void OPL3_EnvelopeCalc(opl3_slot *slot)
{
    ...
    case envelope_gen_num_attack:
        ...
        {
            eg_inc = (   (unsigned)(~slot->eg_rout) << shift) >> 4; //shift fix
        }
        break;
    case envelope_gen_num_decay:
...
void OPL3_GenerateResampled(opl3_chip *chip, /*Bit16s*/ signed int *buf)
{
    ...
    /*buf[0]*/ signed int L = /*(Bit16s)*/ ((chip->oldsamples[0] * (chip->rateratio - chip->samplecnt)
                     + chip->samples[0] * chip->samplecnt) / chip->rateratio);
    /*buf[1]*/ signed int R = /*(Bit16s)*/ ((chip->oldsamples[1] * (chip->rateratio - chip->samplecnt)
                     + chip->samples[1] * chip->samplecnt) / chip->rateratio);

//    buf[0]=(Bit16s)((L+R)/2); //convert to mono
      buf[0]+=L+R;              //Mono, громкость в 2 раза больше
...
void OPL3_GenerateStream(opl3_chip *chip, /*Bit16s*/ signed int *sndptr, Bit32u numsamples)
{
    Bit32u i;
