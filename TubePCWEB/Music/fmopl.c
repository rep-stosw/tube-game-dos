...
INLINE void set_ar_dr(FM_OPL *OPL,int slot,int v)
{
	...
	SLOT->_DR = dr ? &OPL->DR_TABLE[dr<<2] : RATE_0;
	SLOT->evsd = SLOT->_DR[SLOT->ksr];
...
static void init_timetables( FM_OPL *OPL , int ARRATE , int DRRATE )
{
	...
	for (i = 0;i < 64 ;i++){	/* make for overflow area */
		LOG(LOG_WAR,("rate %2d , ar %f ms , dr %f ms \n",i,
			((fltype)(EG_ENT<<ENV_BITS) / OPL->AR_TABLE[i]) * (1000.0 / OPL->rate),
			((fltype)(EG_ENT<<ENV_BITS) / OPL->DR_TABLE[i]) * (1000.0 / OPL->rate) ));
	}
#endif
}
...
/*		LOG(LOG_INF,("sin(%3d) = %f:%f db\n",s,pom,(fltype)j * EG_STEP));*/
	...
	{
		/* ATTACK curve */
		pom = pow( ((fltype)(EG_ENT-1-i)/EG_ENT) , 8 ) * EG_ENT;
	...
	{
		/* 100cent = 1seminote = 6% ?? */
		pom = (fltype)VIB_RATE*0.06*sin(2*PI*i/VIB_ENT); /* +-100sect step */
...
static void OPL_initalize(FM_OPL *OPL)
{
	...
	/* frequency base */
	OPL->freqbase = (OPL->rate) ? ((fltype)OPL->clock / OPL->rate) / 72  : 0;
	/* Timer base time */
	OPL->TimerBase = 1.0/((fltype)OPL->clock / 72.0 );
	/* make time tables */
	...
	/* LFO freq.table */
	OPL->amsIncr = OPL->rate ? (fltype)AMS_ENT*(1<<AMS_SHIFT) / OPL->rate * 3.7 * ((fltype)OPL->clock/3600000) : 0;
	OPL->vibIncr = OPL->rate ? (fltype)VIB_ENT*(1<<VIB_SHIFT) / OPL->rate * 6.4 * ((fltype)OPL->clock/3600000) : 0;
}
	...
	case 0x00: /* 00-1f:controll */
		...
		case 0x04:	/* IRQ clear / mask and Timer enable */
			...
			{	/* set IRQ mask ,timer enable*/
				...
				if(OPL->st[1] != st2)
				{
					fltype interval = st2 ? (fltype)OPL->T[1]*OPL->TimerBase : 0.0;
					OPL->st[1] = st2;
				...
				if(OPL->st[0] != st1)
				{
					fltype interval = st1 ? (fltype)OPL->T[0]*OPL->TimerBase : 0.0;
					OPL->st[0] = st1;
...
/* ---------- update one of chip ----------- */
void YM3812UpdateOne(FM_OPL *OPL, /*INT16*/ signed int *buffer, int length)
{
    int i;
	int data;
	/*OPLSAMPLE*/ signed int *buf = buffer;
...
//		buf[i] = data >> OPL_OUTSB; //mono
                buf[i]+=data>>(OPL_OUTSB-1); //Mono, громкость увеличена в 2 раза

//		buf[ 2*i   ] = data >> OPL_OUTSB; //stereo
//              buf[(2*i)+1] = buf[ 2*i   ];
	}
...
int OPLTimerOver(FM_OPL *OPL,int c)
{
	...
	/* reload timer */
	if (OPL->TimerHandler) (OPL->TimerHandler)(OPL->TimerParam+c,(fltype)OPL->T[c]*OPL->TimerBase);
	return OPL->status>>7;
}
